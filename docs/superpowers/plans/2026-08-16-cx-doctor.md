...
def test_doctor_fail_on_never_always_exits_0(proj, tmp_path, capsys):
...
def test_doctor_ignore_accepts_comma_separated(proj, capsys):
    (proj / ".claude" / "settings.json").write_text("{ broken", encoding="utf-8")
    code = main(["doctor", "--path", str(proj),
                 "--ignore", "schema.json-syntax,refs.nothing", "--json"])
    payload = json.loads(capsys.readouterr().out)
    assert code == 0
    assert payload["summary"]["ignored"] >= 1
