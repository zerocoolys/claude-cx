...
**原因**：拆包后 `discover_sources` 住在 `cx/discovery.py`，它按自己模块的全局名字查找 `managed_dirs`；补丁打在 `cx` 包属性上不会影响它。这是猴补丁目标模块变了，不是行为变更。**除这 2 行外，`tests/test_cx.py` 不得有任何其他改动**——保持不变正是搬运正确性的验收依据。
